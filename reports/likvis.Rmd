...
Test this on synthetic data. Use xs with most data.

```{r}
userow <- which.max(test_smry$n)
test_smry[userow, ]
real_A0 <- test_smry$A0[userow]
real_sigmalogA <- test_smry$logA_sd[userow]

obs <- test_full$dA[test_full$xs == test_smry[userow, ][["xs"]]]
dates <- test_full$datetime[test_full$xs == test_smry[userow, ][["xs"]]]
priors <- stanPriorList[[userow]]
...
ndat <- length(obs)
logposter <- logPost(datavec = obs[1:ndat], muhat = priors[["logA0_hat"]],
